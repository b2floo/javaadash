...
import com.javaadash.tc2.core.Dealer;
...
public class DealerTest extends TestCase
{
	public void testDealPlayerCards() throws Exception {
		...
		assertEquals(maxActions - handsize, inGameDeck.getCards(CardType.ACTION,CardLocation.STOCK).size());
	...
	public void testDealCards() throws Exception {
		...
		assertEquals(maxActions - handsize1, p1.getIngameDeck().getCards(CardType.ACTION,CardLocation.STOCK).size());
		assertEquals(handsize2, p2.getIngameDeck().getCards(CardType.ACTION,CardLocation.HAND).size());
		assertEquals(maxActions - handsize2, p2.getIngameDeck().getCards(CardType.ACTION,CardLocation.STOCK).size());
	}
