...
public class InGameDeckTest extends TestCase {
  ...
  protected void setUp() throws Exception {
    ...
    for (int i = 0; i < maxCharacterrs; i++) {
      Card c = new Card(CardType.CHARACTER);
      chars.add(c);
      deck.addCard(c);

    }
    for (int i = 0; i < maxActions; i++) {
      Card a = new Card(CardType.ACTION);
      actions.add(a);
      deck.addCard(a);
    }
  }
  ...
  public void testCreateInGameDeck() throws Exception {
    ...
    for (Integer cardLocation : MockCardLocation.values()) {
      if (cardLocation == CardLocation.STOCK) {
        assertEquals(maxActions - initialHandSize,
            inGameDeck.getCards(CardType.ACTION, cardLocation).size());
