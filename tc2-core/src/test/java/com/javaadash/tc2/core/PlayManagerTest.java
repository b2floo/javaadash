...
public class PlayManagerTest extends TestCase {
  ...
  /*
   * public void testPlay2Actions() throws Exception { Player p = GameUtils.getPlayer("junit", 0,
   * 2); p.setPlayListener(this);
   * 
   * // put actions in hand and set them chainable for(Card
   * action:p.getIngameDeck().getCards(CardType.ACTION)) { action.setChain(new Chain());
   * p.getIngameDeck().setCardLocation(action, CardLocation.HAND); }
   * 
   * assertEquals(0, p.getIngameDeck().getCards(CardType.ACTION,CardLocation.BOARD).size());
   * GameContext context = new GameContext(); context.setState(GameState.PLAYER_CHOOSE_ACTIONS);
   * p.play(context); assertEquals(2,
   * p.getIngameDeck().getCards(CardType.ACTION,CardLocation.BOARD).size()); }
   */
